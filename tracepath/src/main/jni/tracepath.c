...
(*env)->GetJavaVM(env, &pgJvm);
(*pgJvm)->AttachCurrentThread(pgJvm, &env, NULL);
...
int printf(const char *fmt, ...){
	...
	buffer[cnt] = '\0';
	int lockValue = pthread_mutex_lock(&mutex);
	__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "print lock:>>>>>>%d", lockValue);
	PrintTraceInfo(buffer);
	int unlockValue = pthread_mutex_unlock(&mutex);
	__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "print unlock>>>>>>%d", unlockValue);
	free(buffer);
	va_end(argptr);
	isFirst++;
	return 1;
